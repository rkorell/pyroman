// Code für die EEprom Speicherung
...
//  -------------------------
 ...
 const int EEPROM_MAX_ADDR = 1023;      // der Nano hat 1 KB = 1024 Bytes EEProm
 const int EEPROM_DEL_START_ADDR = 40;  // Wichtige Daten stehen unterhalb 
  					// der 40  und sollen NICHT gelöscht werden
 					// eeprom_erase_all startet dort
 ...
 void eeprom_erase_all(byte b = 0xFF) {
   ...
   for (i = EEPROM_DEL_START_ADDR; i <= EEPROM_MAX_ADDR; i++) {
     EEPROM.write(i, b);
   }
 }
